...
import { Book, ScrollText, PlayCircle, ChevronLeft, Target, Lightbulb, ClipboardList, CheckCircle2 } from 'lucide-react';
import ExperimentSection from '../../components/ExperimentSection';

const PythonExperimentPage = () => {
    const { id } = useParams();
    const navigate = useNavigate();
    const experiment = pythonExperiments.find(e => e.id === parseInt(id));

    const [activeTab, setActiveTab] = useState('theory');

    useEffect(() => {
        if (!experiment) navigate('/python-lab');
    }, [experiment, navigate]);
            ...
            <div className="h-14 w-full shrink-0 border-b border-white/5 flex items-center justify-between px-4 sm:px-6 bg-[#0f172a] z-30">
                <div className="flex items-center gap-2 sm:gap-4 overflow-hidden">
                    ...
                    <h2 className="font-bold text-slate-200 uppercase tracking-tight text-xs sm:text-sm truncate">
                        Python Expt {experiment.id}: {experiment.title}
                    </h2>
                </div>
            </div>
                ...
                <div className="flex-1 w-full relative overflow-hidden bg-[#020617] min-h-0">
                    <AnimatePresence mode="wait">
                        ...
                        {activeTab === 'editor' && (
                            ...
                            >
                                <PythonCompiler defaultCode={experiment.defaultCode} />
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
...
export default PythonExperimentPage;
