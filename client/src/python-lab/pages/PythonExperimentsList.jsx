...
import { pythonExperiments } from '../data/pythonExperiments';
import { motion } from 'framer-motion';
import { Link } from 'react-router-dom';
import { ArrowRight, FlaskConical } from 'lucide-react';
import { GlowingEffect } from '../../components/ui/glowing-effect';

const PythonExperimentsList = () => {
    return (
        <div className="w-full h-full px-2 sm:px-4 py-8 overflow-y-auto scrollbar-hide bg-[#020617]">
            <header className="mb-12 text-center w-full px-4">
                ...
                >
                    <div className="bg-gradient-to-tr from-yellow-400 to-blue-600 p-3 rounded-2xl shadow-xl shadow-yellow-500/20">
                        <FlaskConical className="w-8 h-8 text-white" />
                    </div>
                    <h1 className="text-4xl font-extrabold tracking-tight text-white uppercase">
                        PYTHON PROGRAMMING <span className="gradient-text">LAB</span>
                    </h1>
                </motion.div>
                <p className="text-slate-400 text-lg max-w-2xl mx-auto">
                    Explore the world of Python programming. Select an experiment below to start your journey from basics to advanced concepts.
                </p>
            </header>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-12">
                {pythonExperiments.map((exp, index) => (
                    <motion.div
                        key={exp.id}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                        <div className="relative h-full rounded-[2rem] p-1">
                            ...
                            <div className="relative glass rounded-[1.75rem] p-8 h-full flex flex-col border border-white/5 bg-slate-900/50 backdrop-blur-2xl transition-all duration-300 overflow-hidden">
                                ...
                                <Link
                                    to={`/python-lab/experiment/${exp.id}`}
...
export default PythonExperimentsList;
