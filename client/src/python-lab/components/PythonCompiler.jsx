...
const PythonCompiler = ({ defaultCode }) => {
    ...
    const handleRun = async () => {
        ...
        try {
            ...
            while (statusId <= 2) {
                ...
                else if (statusId === 2) setOutput('Running...');
    ...
    return (
        <div className="flex flex-col h-full w-full overflow-hidden bg-[#020617]">
            <div className="h-12 w-full border-b border-white/5 flex items-center justify-between px-4 bg-[#0f172a] shrink-0">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Python Compiler</span>
                <button
                    onClick={handleRun}
                    disabled={isRunning}
            ...
            <div className="flex-1 flex flex-col lg:flex-row min-h-0">
                <div className="flex-[0.7] border-r border-white/5 h-full relative">
                    <Editor code={code} onChange={setCode} theme="dark" fontSize={16} language="python" />
                </div>
