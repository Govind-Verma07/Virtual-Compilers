...
import { cExperiments } from '../data/cExperiments';
import { motion } from 'framer-motion';
import { Link } from 'react-router-dom';
import { BookOpen, Code2, ArrowRight, Binary } from 'lucide-react';
    ...
    return (
        <div className="w-full h-full px-2 sm:px-4 py-8 overflow-y-auto scrollbar-hide bg-[#020617]">
            <header className="mb-12 text-center w-full px-4">
                ...
                >
                    <div className="bg-gradient-to-tr from-blue-500 to-indigo-600 p-3 rounded-2xl shadow-xl shadow-blue-500/20">
                        <Binary className="w-8 h-8 text-white" />
                    </div>
                    <h1 className="text-4xl font-extrabold tracking-tight text-white">
                        C PROGRAMMING <span className="text-blue-500">LAB</span>
                    </h1>
                </motion.div>
                <p className="text-slate-400 text-lg max-w-2xl mx-auto">
                    Welcome to the Virtual C Programming Laboratory. Explore low-level programming concepts and run code in real-time.
                </p>
            </header>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-12">
                {cExperiments.map((exp, index) => (
                    <motion.div
                        key={exp.id}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
