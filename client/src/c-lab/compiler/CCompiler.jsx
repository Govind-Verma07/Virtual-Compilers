...
import CConsole from '../components/CConsole';
import CInputPanel from '../components/CInputPanel';
import axios from 'axios';
import { PlayCircle } from 'lucide-react';

const SUBMISSION_URL = 'https://ce.judge0.com/submissions?base64_encoded=false&wait=true';

const CCompiler = ({ defaultCode }) => {
        ...
        <div className="flex flex-col h-full w-full overflow-hidden bg-[#020617]">
            <div className="h-12 w-full border-b border-white/5 flex items-center justify-between px-4 bg-[#0f172a] shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">C Online Compiler</span>
                </div>
                <button
                    onClick={handleRun}
                    disabled={isRunning}
            ...
            <div className="flex-1 flex flex-col lg:flex-row min-h-0">
                <div className="flex-[0.7] border-r border-white/5 h-full relative">
                    <CEditor code={code} onChange={setCode} theme="dark" fontSize={16} />
                </div>
                <div className="flex-[0.3] flex flex-col h-full">
                    <div className="flex-1 overflow-hidden border-b border-white/5">
                        <CInputPanel value={userInput} onChange={setUserInput} />
                    </div>
                    <div className="flex-1 overflow-hidden">
                        <CConsole output={output} onClear={() => setOutput('')} isRunning={isRunning} />
                    </div>
                </div>
            </div>
        </div>
